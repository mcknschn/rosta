...
- `c1` makt: andel av 2014–2026 partiet satt i regering (nationellt) + andel region/kommun-styren (Kolada/SKR) som rör kategorin.
- `c2` finansiering: är partiets budgetmotioner saldomässigt finansierade.
...
- **Region/kommun-styren historiskt** saknar en enda ren officiell dataset; SKR:s sammanställningar efter varje val är källan men kräver troligen viss manuell strukturering i `mappings.yaml` (Fas 1-uppgift). Tills datan finns: när subnationell styresdata saknas omviktas C till 100 % nationellt **och** C:s säkerhet sänks ett steg (skiljt från `vikt = 0 per design`, som för försvar) — så bristen redovisas i osäkerheten i stället för att tyst skeva välfärd/integration.
