...
| 1b | ✅ | `a1` budgetprioritering **byggd och aktiv** (gated): partiernas föreslagna utgiftsramar per UO **över budgetåren 2011–2025** ur respektive FiU1-rambeslut, troget transkriberade till `config/budget_ramar.yaml` av `pipeline/tools/budget_ramar_transcribe.py` (ingen runtime-parser; 2023–2025 expertgranskade 2026-06-05, övriga i version 0). Fönstret är samma som förankringens (ADR 0007). a1 = snitt över åren (snitt-skärning av aktiva kategorier). A = 0,6·a1 + 0,4·a2; hård grind kräver alla 8 partier per kategori-UO annars `A=a2`. Adversariellt verifierad: summainvariant per kolumn + betänkandets bilagor (absoluta belopp, tusental kronor) + betänkandets HTML + roll-call; de tre signade åren läses om ur PDF:en och träffar configen på kronan, 405 celler. [Metod](docs/done/fas1b_budget_metod.md). Voteringsprov utökat till hela fönstret (12 riksmöten, sampelt; matar ännu inget betyg). |
