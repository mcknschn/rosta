...
**A — Prioritering** (vad partiet prioriterat, som andel — inte i absolut volym): `A = 0,5·a1 + 0,5·a2`. A mäts i två **kanaler**, och de väger lika sedan [ADR 0015](docs/adr/0015-as-tva-kanaler-vager-lika.md). Kanalerna mäter en storhet, och båda mäts mot sin egen förankring med samma avbildning. Det enda nedskrivna skälet till en asymmetri är prövat och fällt. Namnet är låst av [ADR 0001](docs/adr/0001-a-mater-prioritering.md); "Faktiskt agerande" är retirerat. Sedan [ADR 0005](docs/adr/0005-a-forankras-i-tid-inte-i-faltet.md) är A **absolut**: båda kanalerna mäts mot en historisk förankring i stället för mot de sju andra partierna. För varje parti och kategori gäller `q = (andel − förankring) / (andel + förankring)`, som ligger i [−1, 1] av konstruktion och är 0 vid jämnhöjd; betyget blir `score.net_support_to_score(q)`, samma linjära avbildning som B använder. Förankringarna står i `config/a_forankring.yaml` och gäller fönstret **2011–2025**. [Metod](docs/done/a_forankring_metod.md).
- `a1` budgetprioritering **(byggd, Fas 1b):** andel av partiets föreslagna utgiftsramar (Σ kategorins UO / Σ alla 27 UO). Förankringen är kategorins andel av de **beslutade** utgiftsramarna i bet. FiU1, som medel över fönstret. Ramtalen transkriberas troget ur officiella källor (FiU1 rambeslut + budgetmotioner) till `config/budget_ramar.yaml` med källrad per frame — **ingen runtime-parser** (det finns ingen strukturerad API-väg till anslag per UO; en bräcklig parser fick inte korrumpera A, tyngsta delpoängen). **Hård grind:** a1 vägs in för en (budgetår, kategori) endast när varje parti har verifierad ram för varje kategori-UO i varje år **partiet självt mäts på** ([ADR 0017](docs/adr/0017-a1-laser-forfattarskap-inte-uppslutning.md) punkt 5 lossade räckvidden från per år till per parti); annars `A = a2` (flagga `A_a2_only`). Tolv parti-år vars enda citerbara grund är en röst i rambeslutet är uteslutna ur a1 på `giltighetsfel`, som en klassregel i `config/scoring.yaml`; gemensam motion och regeringsram står kvar, eftersom båda är författarskap. Saknad cell → hård fail, aldrig tyst 0. a1 är ett snitt över **budgetåren 2011–2025** ur respektive FiU1-rambeslut, alltså samma år som förankringen ([ADR 0007](docs/adr/0007-a-mats-over-samma-fonster-som-sin-forankring.md) punkt 1). Fönstret faller ut ur tre gränser som skrevs före hämtningen; före 2011 saknade SD mandat i kammaren och kan därför inte ha någon citerbar ram. Alla femton budgetår är expertgranskade med mänsklig sign-off (2023–2025 den 2026-06-05, 2011–2022 den 2026-08-26). [Metod](docs/done/fas1b_budget_metod.md).
...
**B — Evidens** (hur stark är den belagda effekten i genomsnitt hos de åtgärder partiet driver, justerat för täckning?):
...
- **Anspråket är begränsat tills nämnaren är rättad** ([ADR 0018](docs/adr/0018-bs-medelvarde-bar-inte-anspraket.md) punkt 3, som ändrar ADR 0004 beslut 1). `Σ q` i nämnaren gör talet till ett **medelvärde**, alltså portföljens genomsnittliga effektstyrka och inte den väntade förbättringens storlek. **Känd följd:** en ytterligare åtgärd med belagd positiv effekt kan **sänka** B, nämligen varje gång dess `m` ligger under partiets dittills vägda medel på indikatorn. Mätt 2026-09-14 sänkte en åtgärd som alla åtta partier stödjer sju av åtta partiers välfärdsbetyg. Rättelsen är en normaliserad summa med förhandsbestämd nämnare och byggs i biljett #50 (ADR 0018 punkt 7). Ett likformigt lyft ger bara en post som är **ensam på sin indikator**; delar posten indikator med andra blir rörelsen olika stor och kan byta tecken (ADR 0018 punkt 4).
- **Konsensus kräver alla åtta kodade** (ADR 0018 punkt 5). En post där bara några partier har en position är **partiellt kodad ensidighet** och beskrivs aldrig som konsensus. En **saknad position är uttryckligen okänd** och läses varken som stöd eller som motstånd.
...
- **D-bredd (sedan 2026-06-12, `coverage_shrink`):** D renormaliserar inte längre bort saknade icke-uteslutna undermått — de bidrar neutralt (net 0) i en fast nämnare av undermåttsvikt, så D gör inget oavkortat kategorianspråk på en delmängd av kategorin. Numeratorn är per (parti, kategori); varje uppmätt cell flaggas `D_shrink_<täckt>/<total>` och viktad täckning < 0,75 ger `D_thin_coverage` + sänkt säkerhet. Tunn bredd grindas via `coverage_allowlist.d_thin_breadth_accepted` (i dag endast försvar 70/100). [Spec](docs/done/d_coverage_krympning_spec.md).
...
`a` är `w_a1 × (partiets giltiga budgetår / fönstrets år) + w_a2` när `A_a1_active` står och
0,50 när `A_a2_only` står, alltså den vikt a2 har i A:s blandning
([ADR 0015](docs/adr/0015-as-tva-kanaler-vager-lika.md) punkt 6,
[ADR 0017](docs/adr/0017-a1-laser-forfattarskap-inte-uppslutning.md) punkt 10). Talet är alltså
per parti sedan 2026-09-14, medan **Mättakets** A-del är kvar som kategorikonstant `w_a1 + w_a2`:
taket säger vad modellen kan mäta och aldrig vad partiet driver (ADR 0017 punkt 12).
Två hål räknas 0 täckta i stället för att strykas ur nämnaren: en ej
...
**Två nämnare, inte en.** Flaggorna `B_shrink_*` och `D_shrink_*` bär KRYMPNINGENS nämnare,
alltså den B och D krymper mot neutral med: kategorins fulla undermåttsvikt minus de uteslutna
undermåttens. Ett undermått står utanför antingen för att var och en av dess indikatorer är
utesluten (ADR 0011) eller för att undermåttet bär sitt eget skäl (ADR 0014 punkt 5).
Den nämnaren står orörd av täckningen, så betygen står still. Täckningen räknas däremot över hela
kategorianspråket, så hålet syns i det tal appen visar. Täckning är därmed inte längre samma tal
som `B_shrink_*` bär, vilket ändrade ADR 0008 punkt 5. Flaggorna hette `B_coverage_*` och
`D_coverage_*` fram till ADR 0014 punkt 8, då ordet byttes: de bär inte Täckningen. Metodrutan
namnger varje uteslutet undermått och varje utesluten indikator med sitt skäl, så ett lägre
täckningstal inte läses som att underlaget blivit sämre.

**Mättaket** (`categories[].coverage_ceiling`, ADR 0014) är det högsta tal Täckning kan anta i en
kategori, blandat med samma delpoängvikter. Det är en kategorikonstant, alltså samma tal för alla
åtta partier: det säger vad modellen kan mäta och aldrig vad partiet driver. Ingen cells `coverage`
överstiger sitt Mättak. Ligger kategorins tak under tröskeln bär cellen
`B_thin_category_ceiling`, alltså modellens tystnad; räcker taket men partiets eget tal ligger
under bär den `B_thin_party_coverage`. Exakt en av dem sätts, så säkerheten sänks ett steg.
...
| 3 | 🟡 | Täckningsverktyg (`coverage_report`) + allowlist (`coverage_allowlist.yaml`) + coverage-gate (inget tyst gap); evidensliggaren 47 källverifierade poster, varav 33 matar B efter den symmetriska evidensgrinden (v3 2026-08-23, ADR 0006; demokrati har 2, övriga 4-7); fler D-sektorsadaptrar kvarstår | Fyller återstående undermått, gör luckor explicita |
...
| 4b | ✅ | B-maskineri (`party_positions × evidence_ledger → indicator_effects → B`) byggt och testat. **B utrullad för ALLA 7 kategorier (version 0):** klimat handkurerat (votering bet. 2023/24:MJU5 + koldioxidskatt), övriga via research+verifierings-workflow mot riksdagens fulltext (fångade fabrikat). **Coverage-viktad B** (`2.5 + (B_raw−2.5)·coverage`, tunn täckning flaggas) — annars mättade B vid 5.0. [Metod](docs/done/fas4b_partistandpunkter_metod.md). Ingen fabrikation. |
| 4c | ✅ | **B-differentiering (byggd version 0, sedan expertgranskad → version 2):** rubrik fryst + evidensgrind testtvingad (negativ-grind i version 0-1, symmetrisk sedan version 2 / ADR 0006); alla ståndpunkter **panel-harmoniserade** (19 åtgärdstyper × 8 partier sida vid sida, M/L-asymmetri rättad); liggaren utökad med enda omstridda åtgärdstyp som passerade evidens-/negativ-grinden av 8 (`ny_karnkraft`→effektbrist, Svenska kraftnät; 7 inerta). **130 ståndpunkter** (109 harmoniserade non-klimat + 14 klimat + 7 kärnkraft). Känslighetsanalys ±0.08; 0 admitterade poster med negativ verkan fram till 2026-08-23, då det källstyrda svepet lade den första (`klimatinvesteringsstod_klimatklivet` → utslappsminskning_per_krona, RiR 2019:1). [ROADMAP Fas 4c](docs/done/ROADMAP.md) · [audit](docs/done/fas4c_planB_audit.md) · [kandidatregister](docs/done/fas4c_planA_kandidatregister.md). |
| 1b | ✅ | `a1` budgetprioritering **byggd och aktiv** (gated): partiernas föreslagna utgiftsramar per UO **över budgetåren 2011–2025** ur respektive FiU1-rambeslut, troget transkriberade till `config/budget_ramar.yaml` av `pipeline/tools/budget_ramar_transcribe.py` (ingen runtime-parser; 2023–2025 expertgranskade 2026-06-05, 2011–2022 den 2026-08-26). Fönstret är samma som förankringens (ADR 0007). a1 = snitt över partiets giltiga år (snitt-skärning av aktiva kategorier). A = 0,5·a1 + 0,5·a2; hård grind kräver att varje parti har ram per kategori-UO i varje år partiet mäts på, annars `A=a2` (ADR 0017). Adversariellt verifierad: summainvariant per kolumn + betänkandets bilagor (absoluta belopp, tusental kronor) + betänkandets HTML + roll-call; de tre signade åren läses om ur PDF:en och träffar configen på kronan, 405 celler. [Metod](docs/done/fas1b_budget_metod.md). Voteringsprov utökat till hela fönstret (12 riksmöten, sampelt; matar ännu inget betyg). |
...
| 3 (T3.9) | ✅ | Evidensliggaren (B): **47 källverifierade poster, varav 33 matar B** (demokrati 2, forsvar 4, ekonomi/valfard/trygghet/klimat 5, integration 7) (ursprungligen 29 + `ny_karnkraft`→effektbrist via Svenska kraftnät, Fas 4c; utökad via B2/B3). Källor URL-bekräftade (stickprov manuellt: IFAU 2025:17, SBU 369, Svk Kraftbalansen 2025), version 3 — den symmetriska evidensgrinden tillämpad 2026-08-23 (ADR 0006): 14 poster utlyfta med skäl per post, inte raderade; v2 expertgranskad (sign-off 2026-06-07). Skarp betygsättning aktiv. Påverkar betyg för **alla 7 kategorier** (partiståndpunkter kurerade + panel-harmoniserade, se 4b/4c). |
