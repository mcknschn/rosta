...
// Svensk konvention: decimalkomma, tankstreck i intervall.
...
export function fmtScoreWithCI(score, ci) {
  ...
  if (!Array.isArray(ci) || ci.length !== 2) return `${s} / 5`;
  return `${s} / 5 (${fmtNum(ci[0], 1)}–${fmtNum(ci[1], 1)})`;
}

// 0–5-skala till procent för stapelbredd.
